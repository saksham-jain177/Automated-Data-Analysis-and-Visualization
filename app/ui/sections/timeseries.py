...
def render_timeseries(df: pd.DataFrame):
    ...
    horizon = st.slider("🔮 Periods ahead", 1, 60, 12)
    ...
    if st.button("🚀 Run Forecast", type="primary", use_container_width=True):
        with st.spinner("Forecasting..."):
            try:
                ...
                fig.add_trace(go.Scatter(x=conf.index, y=conf["lower"], mode="lines", fill="tonexty", line=dict(width=0), name="95% CI", fillcolor="rgba(255,0,0,0.2)"))
                fig.update_layout(title=f"Forecast: {value_col}", xaxis_title="Time", yaxis_title=value_col, hovermode="x unified")
                st.plotly_chart(fig, use_container_width=True)
                st.success(f"✓ Predicted {horizon} periods ahead.")
